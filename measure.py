...
def dice_coefficient_withLogtis(y_pred, y_true, soft=True ,eps=1e-7):
...
def iou_score_withLogtis(y_pred, y_true, soft=True ,eps=1e-7):
    y_pred = torch.sigmoid(y_pred)
    if soft:
        y_pred = y_pred.float()
    else:
...
def pixel_accuracy_withLogtis(y_pred, y_true, soft=True):
    y_pred = torch.sigmoid(y_pred)
    y_pred = (y_pred > 0.5).float()
    y_true = y_true.float()
...
def sensitivity_withLogtis(y_pred, y_true, soft=True, eps=1e-7):
...
def specificity_withLogtis(y_pred, y_true, soft=True, eps=1e-7):
...
# Dice metric (masked)
# ---------------------------
def masked_dice_coef(logits, targets, mask, soft=True, eps=1e-6):
...
def masked_iou_score_withLogtis(y_pred, y_true, mask, soft=True, eps=1e-7):
    y_pred = torch.sigmoid(y_pred)
    if soft:
        y_pred = y_pred.float()
    else:    
...
def masked_pixel_accuracy_withLogtis(y_pred, y_true, mask, soft=True):
    y_pred = torch.sigmoid(y_pred)
    y_pred = (y_pred > 0.5).float()
...
def masked_sensitivity_withLogtis(y_pred, y_true, mask, soft=True, eps=1e-7):
    # 在所有真实病变中，有多少被模型正确预测出来 - False Negative错识别为健康的，它真实是病变的
    # True Positive / (True Positive + False Negative)
    y_pred = torch.sigmoid(y_pred)
    if soft:
...
def masked_specificity_withLogtis(y_pred, y_true, mask, soft=True, eps=1e-7):
    # 在所有健康的案例，有多少被模型正确预测出来 - False Positive错识别为病例的，它真实是健康的
    # True Negative / (True Negative + False Positive)
    y_pred = torch.sigmoid(y_pred)
    if soft:
